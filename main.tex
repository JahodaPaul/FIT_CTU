...
We have developed an autonomous driving system that can chase another vehicle using only images from a single RGB camera. At the core of the system is a novel dual-task convolutional neural network capable of simultaneously performing object detection as well as coarse semantic segmentation. The system has been firstly tested in a CARLA simulation. For this reason, we have created a new challenging publicly available chasing dataset collected by manually driving the chased car. Using the dataset, we have shown that the system benefits from using coarse semantic segmentation. The system that uses the semantic segmentation was able to chase the pursued car on average 10\% longer than other versions of the system. 

Finally, we have integrated the system into a subscale vehicle platform attached to a high-speed RC car and demonstrated its capabilities by autonomously chasing another RC car.
...
\begin{figure}[]
    \centering
    \subfloat[First proposed path]{{\includegraphics[width=0.45\textwidth]{images/diagram1.pdf} }}%
    \qquad
    \subfloat[Second proposed path]{{\includegraphics[width=0.45\textwidth]{images/diagram3.pdf} }}%
...
On the other hand, the second set aims to test the algorithm to its limits. The drives include sudden braking, fast turns as well as cutting corners. Sometimes even drifts were performed. The average speed in these drives is 49.68 km/h with the fastest drive having average speed of 63.29 km/h. %The average speed distribution of both of these sets are seen in Figure \ref{f:speed_distribution}.
...
\begin{figure}[h]
    \centering
    \subfloat[Class distribution]{{\includegraphics[width=0.45\textwidth]{images/bar_chart_drivable.pdf} }}%
    \quad
    \subfloat[Confusion matrix. Value P represent Drivable surface while N represents non-drivable surface
]{{\includegraphics[width=0.45\textwidth]{images/confusion_matrix.pdf} }}%
...
\begin{figure}[]
    \centering
    \subfloat[Steer values in time]{{\includegraphics[width=0.45\textwidth]{images/steer_vid.pdf} }}%
    \quad
    \subfloat[Throttle in time]{{\includegraphics[width=0.45\textwidth]{images/throttle_vid.pdf} }}%
...
\begin{figure}[]
    \centering
    % \subfloat[Image taken at $t$ seconds]{{\includegraphics[width=0.9\textwidth]{images/drone_shot_2.4.2-optimized.png} }}%
    % \quad
