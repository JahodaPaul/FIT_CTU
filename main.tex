...
As we can see in Table \ref{tab:detection} the recall of the detector is significantly lower than its precision. In this experiment, we investigate the effects of detectors recall on the ability of the autonomous driving system to chase another vehicle. Furthermore, we investigate how each of the tree versions of the system cope with different detection recall levels. Figure \ref{f:detection_recall} plots the average drive completion (of the 20 drives) described in Section \ref{s:experiment1} against the detection recall. 

% todo - ADD figure
\begin{figure}[h!]
    \centering
    \includegraphics[width=1\textwidth]{images/recall_chart.pdf}
    
    \caption{TODO}\label{f:detection_recall}
\end{figure}
