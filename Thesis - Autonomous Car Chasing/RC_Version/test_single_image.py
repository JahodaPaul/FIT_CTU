...
parser.add_argument("--class_name_path", type=str, default="./model/coco.names",
...
with tf.Session() as sess:
    ...
    for input_image in files:
        ...
        if len(boxes_) != 0:
            dist, angle = getDistanceAndAngle(boxes_[np.argmax(scores_)],width_ori,height_ori)
            x0, y0, x1, y1 = boxes_[np.argmax(scores_)]
            plot_one_box(img_ori, [x0, y0, x1, y1], label=args.classes[labels_[np.argmax(scores_)]] + ', {:.2f}%'.format(scores_[np.argmax(scores_)] * 100), color=color_table[labels_[np.argmax(scores_)]],distance=dist,angle=angle)
