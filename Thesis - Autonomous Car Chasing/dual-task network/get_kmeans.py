...
def kmeans(boxes, k, dist=np.median):
    ...
    while True:
        ...
        #print('Nearest:',nearest_clusters)

        if (last_clusters == nearest_clusters).all():
            break

        for cluster in range(k):
            if not math.isnan(dist(boxes[nearest_clusters == cluster], axis=0)[0]):
...
if __name__ == '__main__':
    ...
    target_size = [320, 320]
    annotation_path = 'data/my_data/train2.txt'#"data/my_data/train.txt"
