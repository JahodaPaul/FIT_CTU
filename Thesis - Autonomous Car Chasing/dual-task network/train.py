...
train_segmentation = tf.Variable(False)#tf.placeholder(tf.bool,name="segmentation")
# train_segm = False
...
train_dataset = train_dataset.map(
    lambda x: tf.py_func(get_batch_data,
                         ...
                         Tout=[tf.int64, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32, tf.bool]),
...
image_ids, image, y_true_13, y_true_26, y_true_52, y_segmentation, train_segmentation = iterator.get_next()
y_true = [y_true_13, y_true_26, y_true_52, y_segmentation]

# tf.data pipeline will lose the data `static` shape, so we need to set it manually
image_ids.set_shape([None])
image.set_shape([None, None, None, 3])
for i in range(3):
    y_true[i].set_shape([None, None, None, None, None])
...
loss = yolo_model.compute_loss(pred_feature_maps, y_true, train_segmentation)
...
#with tf.InteractiveSession() as sess:#.Session() as sess:
...
if True:
    ...
    for epoch in range(args.total_epoches):
        ...
        for i in trange(args.train_batch_num):
            _, summary, __y_pred, __y_true, __loss, __global_step, __lr, train_segmentation__ = sess.run(
                [train_op, merged, y_pred, y_true, loss, global_step, learning_rate,train_segmentation],
                feed_dict={is_training: True})
            # tf.Print(train_segmentation, [train_segmentation], message="Train segmentation: ")

            # print(train_segmentation__)
            ...
            if __global_step % args.train_evaluation_step == 0 and __global_step > 0:
                ...
                recall, precision = evaluate_on_gpu(sess, gpu_nms_op, pred_boxes_flag, pred_scores_flag, __y_pred, __y_true, args.class_num, iou_thresh=args.nms_threshold,segmentation=train_segmentation__)
