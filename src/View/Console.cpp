...
namespace RG {
    ...
    void Console::Draw() {
        ...
        if (ImGui::InputText(m_ret, InputBuf, IM_ARRAYSIZE(InputBuf), ImGuiInputTextFlags_EnterReturnsTrue|ImGuiInputTextFlags_CallbackCompletion|ImGuiInputTextFlags_CallbackHistory,
            ...
            case ImGuiInputTextFlags_CallbackCompletion:
                {
                    ...
                    for ( auto & it : m_buildIn ) {
                        ...
                        if ( res.first == cur.end() ) {
                            ...
                            if ( !first_match ) {
                                ...
                                prefix = prefix.substr( 0, std::min( pref.first - s1->begin(), pref.second - s2->begin() ));
                            }
                        }
                    }
                    for ( auto & it : m_api ) {
                        ...
                        if ( res.first == cur.end() ) {
                            ...
                            if ( !first_match ) {
                                ...
                                if ( prefix.size() < it.first.size() ) {
                                    s1 = &prefix; s2 = &it.first;
                                } else {
                                    s2 = &prefix; s1 = &it.first;
                                }
                                auto pref = std::mismatch( s1->begin(), s1->end(), s2->begin() );
                                prefix = prefix.substr( 0, std::min( pref.first - s1->begin(), pref.second - s2->begin() ));
                            }
                    ...
                    if ( prefix.size() ) {
                        data->CursorPos = data->SelectionStart = data->SelectionEnd = data->BufTextLen = (int)snprintf(data->Buf, (size_t)data->BufSize, "%s", prefix.c_str());
                        data->BufDirty = true;
                    }
                }
